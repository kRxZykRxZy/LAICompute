...
std::string backend_panel(const videocore::DeviceInfo& d) {
    ...
    const std::string html =
        "<div id=\"computeMode\" style=\"border-top:1px solid var(--line);padding-top:12px\">"
        "<div class=\"section-title\"><span>Inference engine</span></div>"
        "<div style=\"display:grid;gap:7px;margin-top:8px\">"
        "<select id=\"backendSelect\" style=\"width:100%;padding:10px;border-radius:12px;background:#151519;color:var(--text);border:1px solid var(--line)\">"
        "<option value=\"cpu\">CPU</option>"
        "<option value=\"gpu\"" + (d.compute_available ? "" : " disabled") + ">GPU / QPU</option>"
        "<option value=\"both\"" + (d.compute_available ? "" : " disabled") + ">CPU + GPU</option>"
        "</select>"
        "<div id=\"gpuInfo\" style=\"font-size:10px;color:var(--muted);line-height:1.4\">" + g +
        (d.present ? " · theoretical " + peak + " GFLOP/s" : "") + "</div></div>"
        "<div style=\"margin-top:10px;padding:10px;border:1px solid var(--line);border-radius:13px;background:#111114\">"
        "<div style=\"font-weight:800;font-size:11px;margin-bottom:7px\">GPU performance</div>"
        "<div id=\"gpuStats\" style=\"font-size:10px;color:var(--muted);line-height:1.55\">Waiting for inference…</div>"
        "<button id=\"gpuBench\" style=\"width:100%;margin-top:8px;padding:8px;border-radius:9px;background:#eee;color:#111;font-size:10px;font-weight:800\">Run GPU benchmark</button>"
        "</div></div>"
        "<script>(function(){const s=document.getElementById('backendSelect'),st=document.getElementById('gpuStats'),bi=document.getElementById('gpuInfo'),bb=document.getElementById('gpuBench');"
        "if(!s)return;const saved=localStorage.getItem('laic-backend');"
        "if(saved&&[...s.options].some(o=>o.value===saved&&!o.disabled))s.value=saved;"
        "async function choose(){localStorage.setItem('laic-backend',s.value);try{await fetch('/api/backend',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({backend:s.value})})}catch(e){}}"
        "s.addEventListener('change',choose);window.LAIC_BACKEND=()=>s.value;"
        "async function stats(){try{const r=await fetch('/api/gpu/stats'),d=await r.json();if(d.available){"
        "const util=d.theoretical_gflops>0?Math.min(100,d.gflops/d.theoretical_gflops*100):0;"
        "st.innerHTML='Kernel: '+d.kernel_ms.toFixed(2)+' ms · '+d.gflops.toFixed(2)+' GFLOP/s<br>Peak utilization: '+util.toFixed(1)+'% · GPU calls: '+d.gpu_calls+' · CPU fallbacks: '+d.fallbacks+'<br>Workgroup: '+d.workgroup_size+' / '+d.max_workgroup_size+' · compute units: '+d.compute_units+' · clock: '+d.clock_mhz+' MHz';"
        "bi.textContent=d.name+' · '+d.runtime+' · '+d.qpus+' QPUs · theoretical '+d.theoretical_gflops.toFixed(2)+' GFLOP/s'"
        "}else st.textContent=d.reason||'GPU runtime unavailable'}catch(e){}}"
        "async function bench(){if(bb.disabled)return;bb.disabled=true;bb.textContent='Benchmarking…';try{"
        "const r=await fetch('/api/benchmark',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({backend:s.value,max_tokens:32})}),d=await r.json();"
        "if(!r.ok)throw Error(d.error||'Benchmark failed');"
        "st.innerHTML='Benchmark: <b>'+d.tokens_per_sec.toFixed(2)+' tok/s</b> · '+d.elapsed_ms.toFixed(0)+' ms · '+d.tokens+' tokens';"
        "if(d.gpu_gflops!==undefined)st.innerHTML+=' · '+d.gpu_gflops.toFixed(2)+' GFLOP/s';"
        "}catch(e){st.textContent=e.message}finally{bb.disabled=false;bb.textContent='Run GPU benchmark'}}"
        "bb.addEventListener('click',bench);stats();setInterval(stats,1000)})();</script>";
    return html;
}
...
struct ApiServer::Impl {
    ...
    double cpu_usage() {
        std::ifstream f("/proc/stat"); std::string label;
        ...
        if (prev_cpu_total) { uint64_t dt=total-prev_cpu_total, di=idle_all-prev_cpu_idle; if(dt) cpu_percent=100.0*double(dt-di)/double(dt); }
        prev_cpu_total=total; prev_cpu_idle=idle_all; return cpu_percent.load();
    }

    std::string models() {
        std::string o="["; bool first=true;
        for(auto& e:std::filesystem::directory_iterator(dir)){if(!e.is_regular_file()||e.path().extension()!=".gguf")continue;if(!first)o+=',';first=false;o+="{\"name\":"+jsonq(e.path().filename().string())+",\"size\":"+std::to_string(std::filesystem::file_size(e.path()))+",\"loaded\":"+(e.path().filename().string()==loaded?"true":"false")+"}";}
        return o+"]";
    }
    ...
    bool begin_generation(LlamaRuntime*& r) {
        std::unique_lock<std::mutex> g(mu); if(!rt||generating)return false;
    ...
    void handle(int c) {
        ...
        std::istringstream q(req);std::string method,path,ver;q>>method>>path>>ver;if(method=="OPTIONS"){writeall(c,reply(200,"{}"));close(c);return;}auto hp=req.find("\r\n\r\n");std::string body=hp==std::string::npos?"":req.substr(hp+4);
        ...
        if(method=="POST"&&path=="/api/model/unload"){std::lock_guard<std::mutex> g(mu);rt.reset();loaded.clear();writeall(c,reply(200,"{\"ok\":true}"));close(c);return;}
        if(method=="GET"&&path=="/api/models"){writeall(c,reply(200,models()));close(c);return;}
        if(method=="POST"&&path=="/api/stop"){std::lock_guard<std::mutex> g(mu);if(rt)rt->stop();writeall(c,reply(200,"{\"ok\":true}"));close(c);return;}
