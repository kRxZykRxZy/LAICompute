...
struct ApiServer::Impl {
    ...
    void handle(int c) {
        ...
        if(method=="POST"&&path=="/api/backend"){auto s=js(body,"backend");auto v=videocore::backend_from_string(s);if((v!=videocore::Backend::CPU)&&!gpu.compute_available){writeall(c,reply(400,"{\"error\":\"GPU backend unavailable\"}"));close(c);return;}selected_backend=v;videocore::set_requested_backend(v);if(rt)rt->set_backend(v);writeall(c,reply(200,backends()));close(c);return;}
        if(method=="POST"&&path=="/api/benchmark"){LlamaRuntime* r=nullptr;if(!begin_generation(r)){writeall(c,reply(409,"{\"error\":\"generation already running or no model loaded\"}"));close(c);return;}auto prompt=js(body,"prompt");if(prompt.empty())prompt="Explain how a Raspberry Pi works in simple terms.";size_t max_tokens=jn(body,"max_tokens",32);std::string out;auto t0=std::chrono::steady_clock::now();try{InferenceConfig cfg;cfg.max_tokens=max_tokens;r->generate_ids(prompt,cfg,[&](uint32_t id){out+=r->tokenizer().decode(id);size_t n=tokens.fetch_add(1)+1;double sec=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();tps=sec>0?n/sec:0;return !r->stop_requested();});double ms=std::chrono::duration<double,std::milli>(std::chrono::steady_clock::now()-t0).count();double gf=0;if(auto*d=r->gpu_stats())if(d->gpu_ns)gf=double(d->gpu_flops)/double(d->gpu_ns);finish_generation();writeall(c,reply(200,"{\"text\":"+jsonq(out)+",\"tokens\":"+std::to_string(tokens.load())+",\"tokens_per_sec\":"+std::to_string(tps.load())+",\"elapsed_ms\":"+std::to_string(ms)+",\"gpu_gflops\":"+std::to_string(gf)+"}"));}catch(const std::exception&e){finish_generation();writeall(c,reply(500,"{\"error\":"+jsonq(e.what())+"}"));}close(c);return;}
        if(method=="POST"&&path=="/api/stop"){std::lock_guard<std::mutex>g(mu);if(rt)rt->request_stop();writeall(c,reply(200,"{\"stopped\":true}"));close(c);return;}
