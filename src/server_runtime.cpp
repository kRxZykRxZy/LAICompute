...
struct ApiServer::Impl{
    ...
    std::atomic<size_t>tokens{0};std::atomic<double>tps{0};std::atomic<double>cpu_percent{0};std::chrono::steady_clock::time_point started;
    uint64_t prev_cpu_total=0,prev_cpu_idle=0;std::mutex cpu_mu;
    Impl(uint16_t p,std::string d):port(p),dir(std::move(d)){std::filesystem::create_directories(dir);}
    ~Impl(){stop();}

    double cpu_usage(){
        std::ifstream f("/proc/stat");std::string label;uint64_t user=0,nice=0,system=0,idle=0,iowait=0,irq=0,softirq=0,steal=0;
        if(!(f>>label>>user>>nice>>system>>idle>>iowait>>irq>>softirq>>steal))return cpu_percent.load();
        uint64_t total=user+nice+system+idle+iowait+irq+softirq+steal;uint64_t idle_all=idle+iowait;std::lock_guard<std::mutex>g(cpu_mu);
    ...
    std::string status(){
        std::ifstream f("/proc/meminfo");std::string k;size_t v,total=0,avail=0;while(f>>k>>v){if(k=="MemTotal:")total=v*1024;else if(k=="MemAvailable:"){avail=v*1024;break;}}
        auto c=CpuInfo::detect();size_t ram=total?100-(avail*100/total):0;double cpu=cpu_usage();
        std::string o="{\"generating\":";o+=generating.load()?"true":"false";o+=",\"tokens\":"+std::to_string(tokens.load())+",\"tokens_per_sec\":"+std::to_string(tps.load());
        o+=",\"cpu_percent\":"+std::to_string(cpu)+",\"ram_percent\":"+std::to_string(ram)+",\"cpu_logical\":"+std::to_string(c.logical_cpus)+",\"cpu_physical\":"+std::to_string(c.physical_cores);
        o+=",\"inference_threads\":"+std::to_string(std::thread::hardware_concurrency())+",\"loaded\":"+jsonq(loaded)+"}";return o;
    }
    ...
    void handle(int c){
        ...
        std::istringstream q(req);std::string method,path,ver;q>>method>>path>>ver;if(method=="OPTIONS"){writeall(c,reply(200,"{}"));close(c);return;}
        ...
        if(method=="POST"&&path=="/api/models/upload"){std::string name=header(req,"X-Filename: ");if(name.empty())name="model.gguf";std::filesystem::path p=std::filesystem::path(dir)/std::filesystem::path(name).filename();if(p.extension()!=".gguf"||body.size()<4||body.compare(0,4,"GGUF")!=0){writeall(c,reply(400,"{\"error\":\"Valid GGUF file required\"}"));close(c);return;}std::ofstream o(p,std::ios::binary|std::ios::trunc);if(!o){writeall(c,reply(500,"{\"error\":\"cannot create model file\"}"));close(c);return;}o.write(body.data(),std::streamsize(body.size()));if(!o){writeall(c,reply(500,"{\"error\":\"failed to save model\"}"));close(c);return;}o.close();writeall(c,reply(201,"{\"name\":"+jsonq(p.filename().string())+",\"size\":"+std::to_string(body.size())+"}"));close(c);return;}
        if(method=="POST"&&path=="/api/models/load"){try{auto name=js(body,"name");auto p=std::filesystem::path(dir)/std::filesystem::path(name).filename();if(name.empty()||p.extension()!=".gguf"||!std::filesystem::exists(p))throw std::runtime_error("model not found");auto x=std::make_unique<LlamaRuntime>();x->load(p.string());std::lock_guard<std::mutex>g(mu);if(generating)throw std::runtime_error("generation in progress");rt=std::move(x);loaded=p.filename().string();writeall(c,reply(200,"{\"ok\":true}"));}catch(const std::exception&e){writeall(c,reply(400,"{\"error\":"+jsonq(e.what())+"}"));}close(c);return;}
