...
namespace { inline float h(const uint8_t*p)noexcept{return Half::from_bits(uint16_t(p[0])|(uint16_t(p[1])<<8)).to_float();} inline void sm4(int j,const uint8_t*q,uint8_t&d,uint8_t&m)noexcept{if(j<4){d=q[j]&63;m=q[j+4]&63;}else{d=(q[j+4]&15)|((q[j-4]>>6)<<4);m=(q[j+4]>>4)|((q[j]>>6)<<4);}} }
float value(GgmlType type,const uint8_t*data,size_t i){const size_t b=i/32,o=i%32;switch(type){
...
case GgmlType::Q5_K:{auto*B=data+(i/256)*176;size_t p=i%256,il=p/64,w=p%64,half=w/32,e=w%32,si=2*il+half;uint8_t sc,m;sm4(int(si),B+4,sc,m);uint8_t ql=B[16+32*il+e/2],qh=B[144+e/2];int nib=half?(ql>>4):(ql&15);int bit=2*il+half;return h(B)*float(sc*(nib+((qh&(1u<<bit))?16:0)))-h(B+2)*float(m);}
