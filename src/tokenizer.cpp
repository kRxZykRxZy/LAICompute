...
std::string cp(uint32_t x){
    std::string s;
    if(x<0x80)s.push_back(char(x));
    else if(x<0x800){s.push_back(char(0xC0|(x>>6)));s.push_back(char(0x80|(x&63)));}
    else if(x<0x10000){s.push_back(char(0xE0|(x>>12)));s.push_back(char(0x80|((x>>6)&63)));s.push_back(char(0x80|(x&63)));}
    else{s.push_back(char(0xF0|(x>>18)));s.push_back(char(0x80|((x>>12)&63)));s.push_back(char(0x80|((x>>6)&63)));s.push_back(char(0x80|(x&63)));}
    return s;
}
...
void Gpt2Tokenizer::load(const GgufModel&m){
    ...
    if(m.str("tokenizer.ggml.model","")!="gpt2")throw std::runtime_error("M7 currently supports GG-2 tokenizers only");
}
std::string Gpt2Tokenizer::byte_encode(const std::string&s){
    ...
    std::unordered_map<int,int>mp;for(int b:direct)mp[b]=b;int extra=0;for(int b=0;b<256;++b)if(!mp.count(b)){mp[b]=256+extra++;}
...
std::string Gpt2Tokenizer::byte_decode(const std::string&s){
    std::vector<int>direct;for(int b=33;b<=126;++b)direct.push_back(b);for(int b=161;b<=172;++b)direct.push_back(b);for(int b=174;b<=255;++b)direct.push_back(b);
    std::unordered_map<int,int>inv;for(int b:direct)inv[b]=b;int extra=0;for(int b=0;b<256;++b)if(std::find(direct.begin(),direct.end(),b)==direct.end())inv[256+extra++]=b;
    std::string out;for(auto&sym:utf8_symbols(s)){uint32_t x=0;for(unsigned char c:sym){if(c<0x80){x=c;break;}if((c&0xE0)==0xC0)x=((c&31)<<6);else if((c&0xF0)==0xE0)x=((c&15)<<12);else if((c&0xF8)==0xF0)x=((c&7)<<18);else if((c&0xC0)==0x80)x|=(c&63);}auto it=inv.find(int(x));if(it!=inv.end())out.push_back(char(it->second));else out+=sym;}return out;
}
std::vector<uint32_t>Gpt2Tokenizer::encode(const std::string&text)const{
    ...
    while(pos<text.size()){
        uint32_t sid=UINT32_MAX;size_t slen=0;for(const auto&kv:ids_)if(kv.first.size()>2&&kv.first.rfind("<|",0)==0&&text.compare(pos,kv.first.size(),kv.first)==0){sid=kv.second;slen=kv.first.size();break;}
        if(sid!=UINT32_MAX){result.push_back(sid);pos+=slen;continue;}
...
std::string Gpt2Tokenizer::decode(uint32_t id)const{
    if(id>=tokens_.size())return{};
    const std::string&t=tokens_[id];
    // Chat-control tokens should not leak into the rendered assistant message.
    if(t.size()>2&&t.rfind("<|",0)==0)return{};
    return byte_decode(t);
}
} // namespace laic
