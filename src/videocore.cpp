...
Generation detect_generation(const std::string& text){
    ...
    if(t.find("2712")!=std::string::npos||t.find("bcm2712")!=std::string::npos||t.find("videoCore vii")!=std::string::npos||t.find("videocore vii")!=std::string::npos)return Generation::VII;
...
Backend backend_from_string(const std::string& value) noexcept{
    ...
    if(v=="gpu"||v=="videocore")return Backend::GPU;
    if(v=="both"||v=="hybrid")return Backend::Both;
    return Backend::CPU;
}
...
DeviceInfo detect(){
    DeviceInfo d;
    std::string all=cpuinfo()+read_file("/sys/firmware/devicetree/base/compatible")+read_file("/sys/firmware/devicetree/base/model");
    d.generation=detect_generation(all);
    if(d.generation==Generation::Unknown)return d;
    d.present=true;
    d.name=generation_name(d.generation);
...
double theoretical_peak_gflops(const DeviceInfo& d) noexcept{
    // Each QPU has four scalar processors and the add/mul ALUs can dual issue.
    return double(d.qpus)*4.0*2.0*double(d.clock_mhz)/1000.0;
}
} // namespace laic::videocore
